...
First, we'll fetch the default application's environment, current application's environment, and the actual geode that maps config values to the environment:
...
```
1. We'll also get the entire application geode:
```shell
...
```
1. Now compare these values to the values contained in the `env` section in application geode:
```shell
